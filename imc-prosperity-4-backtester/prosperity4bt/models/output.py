...
class SandboxLogRow:
    ...
    def with_offset(self, timestamp_offset: int) -> 'SandboxLogRow':
        return SandboxLogRow(self.timestamp + timestamp_offset, self.sandbox_log, self.lambda_log.replace(f'[[{self.timestamp},', f'[[{self.timestamp + timestamp_offset},'))

    def __str__(self) -> str:
        return orjson.dumps({'sandboxLog': self.sandbox_log, 'lambdaLog': self.lambda_log, 'timestamp': self.timestamp}, option=orjson.OPT_APPEND_NEWLINE | orjson.OPT_INDENT_2).decode('utf-8')

    def to_dict(self):
        return {'sandboxLog': self.sandbox_log, 'lambdaLog': self.lambda_log, 'timestamp': self.timestamp}
...
class TradeRow:
    ...
    def with_offset(self, timestamp_offset: int) -> 'TradeRow':
        return TradeRow(Trade(self.trade.symbol, self.trade.price, self.trade.quantity, self.trade.buyer, self.trade.seller, self.trade.timestamp + timestamp_offset))

    def to_dict(self):
        return {'timestamp': self.trade.timestamp, 'buyer': self.trade.buyer, 'seller': self.trade.seller, 'symbol': self.trade.symbol, 'currency': 'XIREC', 'price': self.trade.price, 'quantity': self.trade.quantity}
...
class BacktestResult:
    ...
    def to_dict(self) -> dict:
        return {'submissionId': str(uuid.uuid4()), 'activitiesLog': ActivityLogRow.get_header_str() + '\n' + '\n'.join([str(al) for al in self.activity_logs]), 'logs': [sl.to_dict() for sl in self.sandbox_logs], 'tradeHistory': [t.to_dict() for t in self.trades]}
