...
class BackDataReader:
    ...
    def read_from_file(self, round_num: int, day_num: int) -> BacktestData:
        ...
        backtest_data = BacktestData(round_num=round_num, day_num=day_num, prices=prices_by_timestamp, trades=trades_by_timestamp, observations=observations_by_timestamp, products=products, profit_loss=profit_loss)
    ...
    def __get_trades(self, round_num: int, day_num: int):
        ...
        with self._read_file_content([f'round{round_num}', f'trades_round_{round_num}_day_{day_num}.csv']) as file:
            if file is not None:
                for line in file.read_text(encoding='utf-8').splitlines()[1:]:
                    columns = line.split(';')
                    trades.append(Trade(symbol=columns[3], price=int(float(columns[5])), quantity=int(columns[6]), buyer=columns[1], seller=columns[2], timestamp=int(columns[0])))
        return trades
