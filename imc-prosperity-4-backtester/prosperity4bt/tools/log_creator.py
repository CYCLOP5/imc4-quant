...
class ActivityLogCreator:
    ...
    def create_log(self) -> list[ActivityLogRow]:
        ...
        for product in self.data.products:
            ...
            columns = [self.day_num, self.state.timestamp, product, row.bid_prices[0] if bid_prices_len > 0 else '', row.bid_volumes[0] if bid_volumes_len > 0 else '', row.bid_prices[1] if bid_prices_len > 1 else '', row.bid_volumes[1] if bid_volumes_len > 1 else '', row.bid_prices[2] if bid_prices_len > 2 else '', row.bid_volumes[2] if bid_volumes_len > 2 else '', row.ask_prices[0] if ask_prices_len > 0 else '', row.ask_volumes[0] if ask_volumes_len > 0 else '', row.ask_prices[1] if ask_prices_len > 1 else '', row.ask_volumes[1] if ask_volumes_len > 1 else '', row.ask_prices[2] if ask_prices_len > 2 else '', row.ask_volumes[2] if ask_volumes_len > 2 else '', row.mid_price, product_profit_loss]
