...
OTM_PINNED = {'VEV_6000': 6000, 'VEV_6500': 6500}
ALL_VOUCHER_STRIKES = {**DEEP_ITM_STRIKES, **SMILE_STRIKES, **OTM_PINNED}
VOUCHERS = list(DEEP_ITM_STRIKES.keys()) + list(SMILE_STRIKES.keys()) + list(OTM_PINNED.keys())
PRODUCTS = [HYDRO, VELVET] + VOUCHERS
LIMITS = {HYDRO: 200, VELVET: 200, **{v: 300 for v in VOUCHERS}}
FV_ALPHA_HYDRO = 0.05
FV_ALPHA_VELVET = 0.1
PREM_ALPHA = 0.05
PREM_VAR_ALPHA = 0.02
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if velvet_mid is not None:
            for name, K in SMILE_STRIKES.items():
                ...
                if ol:
                    orders[name] = ol
        for name, K in OTM_PINNED.items():
            od = state.order_depths.get(name)
            if od is None:
                continue
            ol = self._trade_otm_pinned(name, K, od, state.position.get(name, 0))
            if ol:
                orders[name] = ol
        return (orders, 0, self._dump_state())
    ...
    def _trade_hydro(self, od: OrderDepth, pos: int) -> List[Order]:
        ...
        o.extend(self._layer_orders(HYDRO, 'B', pb, br, bb, ba))
        o.extend(self._layer_orders(HYDRO, 'S', pa, sr, bb, ba))
        return o
    ...
    def _trade_velvet(self, od: OrderDepth, pos: int) -> List[Order]:
        ...
        fv = self.fv_v + trend_nudge + flow_nudge
        inv_frac = pos / ml
        floor = 0.6
        if abs(inv_frac) > floor:
            s = 1 if inv_frac > 0 else -1
    ...
    def _trade_deep_itm(self, name: str, K: int, od: OrderDepth, pos: int) -> List[Order]:
        ...
        o.extend(self._layer_orders(name, 'B', pb, br, bb, ba))
        o.extend(self._layer_orders(name, 'S', pa, sr, bb, ba))
        return o
    ...
    def _trade_smile(self, name: str, K: int, od: OrderDepth, pos: int, S: float) -> List[Order]:
        ...
        if not SMILE_MAKE_ENABLED:
            return o
        pb, pa = self._quote_inside_or_at(mid, bb, ba, SMILE_MAKE_EDGE, pos, ml, SMILE_SKEW)
    ...
    def _trade_otm_pinned(self, name: str, K: int, od: OrderDepth, pos: int) -> List[Order]:
        ml = LIMITS[name]
        br = ml - pos
        sr = ml + pos
        o: List[Order] = []
        ...
        if ba - bb < 1:
            return o
        skew_frac = pos / ml
        buy_sz = int(br * max(0.0, 1.0 - skew_frac))
        sell_sz = int(sr * max(0.0, 1.0 + skew_frac))
        if buy_sz > 0:
            o.append(Order(name, bb, buy_sz))
        if sell_sz > 0:
            o.append(Order(name, ba, -sell_sz))
        return o
    ...
    def _dump_state(self) -> str:
        return json.dumps({'fv_h': round(self.fv_h, 4), 'fv_v': round(self.fv_v, 4), 'sfv': round(self.slow_fv_v, 4), 'sfh': round(self.slow_fv_h, 4), 'fl': round(self.flow_v, 3), 'pe': {k: round(v, 4) for k, v in self.prem_ema.items()}, 'pv': {k: round(v, 6) for k, v in self.prem_var_ema.items()}, 'lm': {k: round(v, 2) for k, v in self.last_mid.items()}}, separators=(',', ':'))
