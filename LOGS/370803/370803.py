...
class Trader:
    ...
    def _dump_state(self) -> str:
        return json.dumps({'fv_h': round(self.fv_h, 4), 'fv_v': round(self.fv_v, 4), 'sfv': round(self.slow_fv_v, 4), 'fl': round(self.flow_v, 3), 'pe': {k: round(v, 4) for k, v in self.prem_ema.items()}, 'pv': {k: round(v, 6) for k, v in self.prem_var_ema.items()}, 'lm': {k: round(v, 2) for k, v in self.last_mid.items()}}, separators=(',', ':'))
