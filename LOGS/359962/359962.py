import json
from typing import List, Dict
from datamodel import OrderDepth, TradingState, Order
lim = {'ASH_COATED_OSMIUM': 80, 'INTARIAN_PEPPER_ROOT': 80}
efv = 10000

class Trader:

    def __init__(self):
        self.k_p = 0.0
        self.k_v = 0.0
        self.k_p00 = 1.0
        self.k_p01 = 0.0
        self.k_p10 = 0.0
        self.k_p11 = 1.0
        self.k_q00 = 0.1
        self.k_q01 = 0.0
        self.k_q10 = 0.0
        self.k_q11 = 0.01
        self.k_r = 2.0
        self.k_h = 15.0

    def bid(self):
        return 1000

    def run(self, state: TradingState):
        o: Dict[str, List[Order]] = {}
        if state.traderData:
            try:
                td = json.loads(state.traderData)
                if isinstance(td, dict):
                    self.k_p = td.get('k_p', self.k_p)
                    self.k_v = td.get('k_v', self.k_v)
                    self.k_p00 = td.get('k_p00', self.k_p00)
                    self.k_p01 = td.get('k_p01', self.k_p01)
                    self.k_p10 = td.get('k_p10', self.k_p10)
                    self.k_p11 = td.get('k_p11', self.k_p11)
            except Exception:
                pass
        if state.timestamp >= 995000:
            print('eod_liq ts=' + str(state.timestamp))
            for prod in state.order_depths:
                pos = state.position.get(prod, 0)
                if pos == 0:
                    continue
                od = state.order_depths[prod]
                eod_orders: List[Order] = []
                if pos > 0:
                    remaining = pos
                    for bp in sorted(od.buy_orders, reverse=True):
                        if remaining <= 0:
                            break
                        q = min(od.buy_orders[bp], remaining)
                        if q > 0:
                            eod_orders.append(Order(prod, bp, -q))
                            remaining -= q
                    if remaining > 0:
                        if od.buy_orders:
                            rest_px = min(od.buy_orders) - 1
                        elif od.sell_orders:
                            rest_px = min(od.sell_orders) - 1
                        else:
                            rest_px = None
                        if rest_px is not None:
                            eod_orders.append(Order(prod, rest_px, -remaining))
                else:
                    remaining = -pos
                    for ap in sorted(od.sell_orders):
                        if remaining <= 0:
                            break
                        q = min(-od.sell_orders[ap], remaining)
                        if q > 0:
                            eod_orders.append(Order(prod, ap, q))
                            remaining -= q
                    if remaining > 0:
                        if od.sell_orders:
                            rest_px = max(od.sell_orders) + 1
                        elif od.buy_orders:
                            rest_px = max(od.buy_orders) + 1
                        else:
                            rest_px = None
                        if rest_px is not None:
                            eod_orders.append(Order(prod, rest_px, remaining))
                if eod_orders:
                    o[prod] = eod_orders
            td_out = {'k_p': self.k_p, 'k_v': self.k_v, 'k_p00': self.k_p00, 'k_p01': self.k_p01, 'k_p10': self.k_p10, 'k_p11': self.k_p11}
            return (o, 0, json.dumps(td_out))
        for prod in state.order_depths:
            if prod == 'ASH_COATED_OSMIUM':
                ol = self.trade_aco(state.order_depths[prod], state.position.get(prod, 0), lim[prod])
                if ol:
                    o[prod] = ol
            elif prod == 'INTARIAN_PEPPER_ROOT':
                ol = self.trade_ipr(state.order_depths[prod], state.position.get(prod, 0), lim[prod])
                if ol:
                    o[prod] = ol
        td_out = {'k_p': self.k_p, 'k_v': self.k_v, 'k_p00': self.k_p00, 'k_p01': self.k_p01, 'k_p10': self.k_p10, 'k_p11': self.k_p11}
        return (o, 0, json.dumps(td_out))

    def trade_aco(self, od: OrderDepth, pos: int, ml: int) -> List[Order]:
        o: List[Order] = []
        br = ml - pos
        sr = ml + pos
        for ap in sorted(od.sell_orders):
            if br <= 0:
                break
            if ap <= efv - 3:
                q = min(-od.sell_orders[ap], br)
                o.append(Order('ASH_COATED_OSMIUM', ap, q))
                br -= q
        for bp in sorted(od.buy_orders, reverse=True):
            if sr <= 0:
                break
            if bp >= efv + 5:
                q = min(od.buy_orders[bp], sr)
                o.append(Order('ASH_COATED_OSMIUM', bp, -q))
                sr -= q
        bb = max(od.buy_orders) if od.buy_orders else efv - 8
        ba = min(od.sell_orders) if od.sell_orders else efv + 8
        edge = 1
        adj_mid = efv - pos / ml * 3.0
        ideal_bid = int(adj_mid - edge)
        ideal_ask = int(adj_mid + edge)
        pb = min(ideal_bid, bb + 1)
        pa = max(ideal_ask, ba - 1)
        if br > 0:
            o.append(Order('ASH_COATED_OSMIUM', pb, br))
        if sr > 0:
            o.append(Order('ASH_COATED_OSMIUM', pa, -sr))
        return o

    def trade_ipr(self, od: OrderDepth, pos: int, ml: int) -> List[Order]:
        o: List[Order] = []
        br = ml - pos
        sr = ml + pos
        if not od.buy_orders or not od.sell_orders:
            return o
        best_bid = max(od.buy_orders.keys())
        best_ask = min(od.sell_orders.keys())
        mid = (best_bid + best_ask) / 2.0
        if self.k_p == 0.0:
            self.k_p = mid
        p_pred = self.k_p + self.k_v
        v_pred = self.k_v
        P00_pred = self.k_p00 + self.k_p10 + self.k_p01 + self.k_p11 + self.k_q00
        P01_pred = self.k_p01 + self.k_p11 + self.k_q01
        P10_pred = self.k_p10 + self.k_p11 + self.k_q10
        P11_pred = self.k_p11 + self.k_q11
        y = mid - p_pred
        S = P00_pred + self.k_r
        K0 = P00_pred / S
        K1 = P10_pred / S
        self.k_p = p_pred + K0 * y
        self.k_v = v_pred + K1 * y
        self.k_p00 = P00_pred - K0 * P00_pred
        self.k_p01 = P01_pred - K0 * P01_pred
        self.k_p10 = P10_pred - K1 * P00_pred
        self.k_p11 = P11_pred - K1 * P01_pred
        predicted_future = p_pred + v_pred * self.k_h
        if predicted_future > best_ask + 3.5:
            for p in sorted(od.sell_orders):
                if br > 0 and p < predicted_future:
                    q = min(-od.sell_orders[p], br)
                    o.append(Order('INTARIAN_PEPPER_ROOT', p, q))
                    br -= q
                else:
                    break
        if predicted_future < best_bid - 3.5:
            for p in sorted(od.buy_orders, reverse=True):
                if sr > 0 and p > predicted_future:
                    q = min(od.buy_orders[p], sr)
                    o.append(Order('INTARIAN_PEPPER_ROOT', p, -q))
                    sr -= q
                else:
                    break
        bv1 = od.buy_orders.get(best_bid, 0)
        av1 = -od.sell_orders.get(best_ask, 0)
        tot = bv1 + av1
        imb = (bv1 - av1) / tot if tot > 0 else 0.0
        shade = 0
        if abs(imb) >= 0.4:
            shade = int(round(imb * 2.0))
        adj_mid = predicted_future - pos / 80.0 * 2.0
        edge = 2
        ideal_bid = int(adj_mid - edge + shade)
        ideal_ask = int(adj_mid + edge + shade)
        pb = min(ideal_bid, best_bid + 1)
        pa = max(ideal_ask, best_ask - 1)
        bsz = br
        ssz = sr
        if imb >= 0.6 and br > 0 and (sr > 0):
            ssz = max(1, int(sr * 0.5))
        elif imb <= -0.6 and br > 0 and (sr > 0):
            bsz = max(1, int(br * 0.5))
        if br > 0:
            o.append(Order('INTARIAN_PEPPER_ROOT', pb, bsz))
        if sr > 0:
            o.append(Order('INTARIAN_PEPPER_ROOT', pa, -ssz))
        return o
